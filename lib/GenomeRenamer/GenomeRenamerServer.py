...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'GenomeRenamer'):
...
impl_GenomeRenamer = GenomeRenamer(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['GenomeRenamer.rename_genome_async'] = ['GenomeRenamer', 'rename_genome']
async_check_methods['GenomeRenamer.rename_genome_check'] = ['GenomeRenamer', 'rename_genome']
sync_methods['GenomeRenamer.rename_genome'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'GenomeRenamer'
        ...
        self.rpc_service.add(impl_GenomeRenamer.rename_genome,
                             name='GenomeRenamer.rename_genome',
                             types=[dict])
        self.method_authentication['GenomeRenamer.rename_genome'] = 'required'
        self.rpc_service.add(impl_GenomeRenamer.status,
                             name='GenomeRenamer.status',
                             types=[dict])
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "GenomeRenamer but no authentication header was passed"
                            raise err
